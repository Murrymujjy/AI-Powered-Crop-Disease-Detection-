...
# Create a sample DataFrame as a knowledge base. In a real app, this would come from a database.
data = {
...
selected_crop = st.sidebar.selectbox("Select your crop:", recommendations_df['Crop'].unique())

if selected_crop:
    filtered_df = recommendations_df[recommendations_df['Crop'] == selected_crop]
    st.sidebar.subheader(f"Common issues for {selected_crop}:")
    selected_issue = st.sidebar.selectbox("Select an issue:", filtered_df['Common Issue'].unique())
